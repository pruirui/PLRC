...
class PaPiLoss(nn.Module):
    ...
    def update_weight_byclsout1(self, cls_predicted_score, batch_index, batch_partial_Y):
        with torch.no_grad():
            ...
            revisedY_raw = revisedY_raw / revisedY_raw.sum(dim = 1).repeat(cls_predicted_score.shape[-1], 1).transpose(0, 1)
    ...
    def update_weight_byclsout1_alim(self, cls_pseudo_label, batch_index):
        with torch.no_grad():
