...
def get_data(data_root, base_dir, num_label=2, random_for_one=0):
    import json
    from collections import defaultdict
    import torch
    import random
    save_path = os.path.join(data_root, base_dir, f'{base_dir}_selected_num_{num_label}_random_one{random_for_one}.pt')
    ...
    for i, (file_path, img) in enumerate(imgs.items()):
        ...
        gts.append(gt)
        py_label = [label2idx[v] for v in random.sample(label_list,  min(len(label_list), 1) if random_for_one > random.random() else min(len(label_list), num_label))]
    ...
    torch.save({'train_data': train_data,
                ...
                'mean_cls': cls,
                'labels': labels
                }, os.path.join(data_root, base_dir, f'{base_dir}_selected_num_{num_label}_random_one{random_for_one}.pt'))

    return train_data, train_gts, train_pys, test_data, test_gts

def load_benthic_dataset(data_root='../data', num_selected=3, train=True, transform=None):
    base_dir = 'Benthic'
    train_data, train_gts, train_pys, test_data, test_gts = get_data(data_root, base_dir, num_label=num_selected, random_for_one=0.6)
    train_gts = torch.Tensor(train_gts).long()
    test_gts = torch.Tensor(test_gts).long()
    if train:
        return Benthic(image_files=train_data, given_partial_label_matrix=train_pys, true_labels=train_gts, test_transform=transform)
    else:
        return Benthic(image_files=test_data, given_partial_label_matrix=None, true_labels=test_gts, test_transform=transform)

def load_benthic(batch_size, data_root='../data', num_selected=2):
    base_dir = 'Benthic'
    ...
    test_dataset = Benthic(image_files=test_data, given_partial_label_matrix=None, true_labels=test_gts, test_transform=test_transform)
    test_loader = torch.utils.data.DataLoader(
        dataset=test_dataset, batch_size=batch_size * 4, \
        shuffle=False, num_workers=8, pin_memory=False
    )

    partial_training_dataset = Benthic(image_files=train_data, given_partial_label_matrix=train_pys, true_labels=train_gts)
